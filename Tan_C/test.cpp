...
void test2()
{
    char *a[] = {"hello","nihao","hi"};
    char **s = a;

    // char **ss = {"hello","nihao","hi"};此处会出错是因为
    // ss 变量中保存的是一个无效随机不可用的地址， 谁也不知道它指向哪里。。。。，*ss 操作会崩溃。。
...
int main()
{
   ...
   */
    ...
    */
    ...
    // cout<<arr+1<<endl;
    // cout<<arr+5<<endl;
    // for(int i = 0;i<4;i++)//此处无法成功执行的原因在于a[i]都是指针，而字符指针如果想要对其赋值的化都需要指向常量，但是我这里的str并非是常量，所以赋值不起
    // {
    //     char str[10] ;
    //     cin>>str;
    //     arr[i] = str;
